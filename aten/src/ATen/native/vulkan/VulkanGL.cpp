// No ATen deps
// GL deps, as its android maybe move that part to /android/ ?
...
void hostCHW_to_deviceTex(
  ...
  auto shader = getShader(
      "nchw_buf_to_tex_glsl", at::native::vulkan::gl::nchw_buf_to_tex_glsl);
  shader->useProgram();
...
double deviceTex2hostCHW(
  ...
  auto program = getShader(
      "tex_to_nchw_buf_glsl", at::native::vulkan::gl::tex_to_nchw_buf_glsl);
  program->useProgram();
...
VulkanGLTensor::VulkanGLTensor(std::vector<int64_t> sizes, const float* data)
    : sizes_(sizes) {
  COUT_FLF;
  assert(sizes_.size() == 4);
  initGLContextOnce();
...
void upsample_nearest2d(
  ...
  auto program = getShader(
      shaderKey, at::native::vulkan::gl::upsampleNearest2d_glsl, header);
...
void add(
  ...
  auto binAddProgram =
      getShader(shaderKey, at::native::vulkan::gl::binary_add_glsl, prefix);
...
void conv2d(
  ...
  auto convProgram = getShader(
      shaderKey, at::native::vulkan::gl::conv_tex_IKnc4hw_glsl, header);
